...
$context['requestor'] = $_SERVER['REMOTE_ADDR'];
$logger = new \Balsama\BostonNeighborhoodFinder\NeighborhoodRequestLogger();

$response = [];
if (!$_POST) {
    http_response_code(400);
    echo '<p>POST a <code>lat</code> and <code>long</code> value to look up tickets.</p>';
    $logger->logRequest('No POST data', $context);
    exit;
}
...
if (array_key_exists('errors', $response)) {
    ...
    $logger->logRequest('Lat and/or Long not posted.', $context);
    exit;
}
...
if (!is_float((float) $_POST['long'])) {
    $context['coordinates_sent']['long'] = $_POST['long'];
...
if (array_key_exists('errors', $response)) {
    echo json_encode($response);
    $context['errors'] = $response['errors'];
    $logger->logRequest('Lot and/or Long outside of bounds.', $context);
    exit;
}
